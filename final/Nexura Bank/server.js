...
app.patch('/api/transfer', (req, res) => {
  const { asset, total, mode } = req.body;
  const sid = req.cookies.sid;
  const username = sid ? sessions.getSessionUser(sid) : '';
  ...
  const transferStatus = userInfo.getUserData(username).transfer(asset, parsedTotal, mode);
...
app.patch('/api/send', (req, res) => {
  const { asset, total, receiver } = req.body;
  const sid = req.cookies.sid;
  const username = sid ? sessions.getSessionUser(sid) : '';
  ...
  const parsedTotal = parseFloat(total);
  const sendStatus = userInfo.getUserData(username).send(asset, parsedTotal);
  ...
  userInfo.getUserData(receiver).receive(asset, parsedTotal);
...
app.patch('/api/convert', (req, res) => {
  const { fromAsset, toAsset, total, exchangeRateToUSD } = req.body;
  const sid = req.cookies.sid;
  const username = sid ? sessions.getSessionUser(sid) : '';
  ...
  const parsedTotal = parseFloat(total);
  const convertStatus = userInfo.getUserData(username).convert({ fromAsset, toAsset, parsedTotal, exchangeRateToUSD });
